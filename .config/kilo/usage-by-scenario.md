...
### 4. `brainstorming`、`decision-framing`、`项目分析` 与 `writing-plans` 都不是独立 command

- `Discovery` 是总阶段名
- 已有项目语境下，`Discovery` 通常表现为“项目分析”
- 新项目语境下，`Discovery` 通常表现为 `brainstorming`
- `decision-framing` 是 `Discovery` 与 `Planning` 之间的可选收敛增强动作，不是新的 phase
...
```text
项目分析 → 必要时 decision-framing → writing-plans → 进入实现 / 调试 / review workflow → 验证与收口
```
...
```text
brainstorming → 必要时 decision-framing → writing-plans → scaffold-project / 实施 → 验证与收口
```
...
- `decision-framing` 是流程增强节点，不单列为 phase
...
- 需要在输出 `writing-plans` 前做一轮结构化收敛与方案取舍
